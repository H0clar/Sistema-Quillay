...
INSERT INTO Asignatura (Nombre, CursoID) VALUES
    ...
    ('Educacion Fisica y Salud', 13),
    ('Historia, Geografia y Ciencias Sociales', 13),
    ('Ingles', 13),
...
FOR EACH ROW
BEGIN
    -- Verificar y corregir el formato del RUT (añadir guion medio si no lo tiene)
    IF NEW.RutUsuario NOT LIKE '%-%' THEN
        SET NEW.RutUsuario = CONCAT(LEFT(NEW.RutUsuario, LENGTH(NEW.RutUsuario) - 1), '-', RIGHT(NEW.RutUsuario, 1));
    END IF;
...
-- Cambios en la tabla Material_Educativo --
